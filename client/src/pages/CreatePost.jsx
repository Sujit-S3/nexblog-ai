...
export default function CreatePost() {
  ...
  // Check if draft arrived from AIWorkspace or LocalStorage
  useEffect(() => {
    const aiDraft = location.state?.aiDraft || sessionStorage.getItem('nexblog_ai_draft');
    const aiTopic = location.state?.topic;
    if (aiDraft) {
      setFormData(prev => ({
        ...prev,
        title: aiTopic || prev.title || 'Flagship AI Draft',
        content: aiDraft.includes('<') ? aiDraft : `<p>${aiDraft.replace(/\n\n/g, '</p><p>')}</p>`,
      }));
      sessionStorage.removeItem('nexblog_ai_draft');
      return;
    }

    // Check autosave
    const saved = localStorage.getItem('nexblog_autosave_create');
    if (saved) {
      try {
        const parsed = JSON.parse(saved);
        if (parsed.title || parsed.content) {
          setFormData(parsed);
          setLastSaved('Restored from autosave');
        }
      } catch (err) {
  ...
  useEffect(() => {
    const timer = setInterval(() => {
      if (formData.title || formData.content) {
        localStorage.setItem('nexblog_autosave_create', JSON.stringify(formData));
        const now = new Date();
  ...
  const handleUploadImage = async () => {
    try {
      ...
      uploadTask.on('state_changed',
        ...
        () => { getDownloadURL(uploadTask.snapshot.ref).then((downloadURL) => { setImageUploadProgress(null); setFormData({ ...formData, image: downloadURL }); }); }
      );
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const data = await apiClient.post('/api/post/create', formData);
      setPublishError(null);
      localStorage.removeItem('nexblog_autosave_create');
      navigate(`/post/${data.slug}`);
    } catch (error) {
      setPublishError(error.message || 'Something went wrong while publishing the article.');
    }
  };
  ...
  // Extract H2 headings for TOC outline
        ...
        <div className='flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8 pb-5 border-b border-slate-200/60 dark:border-slate-800'>
          <div>
            <div className='flex items-center gap-2'>
              <h1 className='font-heading text-3xl font-bold text-slate-900 dark:text-white'>Flagship Article Studio</h1>
              <span className='badge-primary text-[10px] uppercase font-bold px-2 py-0.5'>Notion AI + Medium</span>
            </div>
            <p className='text-xs text-slate-500 dark:text-slate-400 mt-1 flex items-center gap-2'>
              <span>Write, audit SEO, and apply multi-model AI commands</span>
              {lastSaved && (
          ...
          <div className='xl:col-span-8 space-y-6'>
            <form onSubmit={handleSubmit} className='space-y-6' id='create-post-form'>
              ...
              <div className='glass dark:glass-dark rounded-2xl p-5 shadow-card'>
                ...
                {imageUploadError && (
                  <p className='text-xs font-medium text-red-500 mt-2'>{imageUploadError}</p>
                )}
                {formData.image && (
              ...
              <button type='submit' className='btn-primary px-10 py-4 text-base font-semibold shadow-glow flex items-center gap-2' id='publish-post-btn'>
                <HiSparkles className='w-5 h-5' />
                <span>Publish Article to Studio</span>
              </button>
            </form>
          </div>
